...
export default function StudentRegisterPage() {
  ...
  } = useForm<StudentRegisterInput>({ resolver: zodResolver(studentRegisterSchema) });

  async function onSubmit(data: StudentRegisterInput) {
    setServerError(null);
    const fd = new FormData();
    Object.entries(data).forEach(([k, v]) => fd.set(k, v));
    const result = await registerStudentAction(fd);
    if (!result.success) {
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/5 p-8 shadow-2xl backdrop-blur-md">
      <h2 className="mb-2 text-center font-heading text-xl font-semibold text-white">
        Daftar sebagai Pelajar / Belia
      </h2>
      <p className="mb-6 text-center text-sm text-white/60">
        Cari peluang kerja dan program latihan di Johor
      </p>

      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
        {[
          { id: "name",            label: "Nama Penuh",          type: "text",     placeholder: "Nama anda",          autoComplete: "name" },
          { id: "email",           label: "Alamat Emel",         type: "email",    placeholder: "nama@contoh.com",    autoComplete: "email" },
          ...
          <div key={field.id}>
            ...
            <input
              ...
              {...register(field.id as keyof StudentRegisterInput)}
            ...
            />
            {errors[field.id as keyof StudentRegisterInput] && (
              <p className="mt-1 text-xs text-johor-red-300">
                {errors[field.id as keyof StudentRegisterInput]?.message}
              </p>
            )}
          </div>
        ))}
